...
func Handler() error {
	...
	for i := 0; i < len(coin.Results); i++ {
		...
		for j := 0; j < len(itr.Coins); j++ {
			...
			if itr.Coins[j].ExpiryDate.Day() == time.Now().Day() && itr.Coins[j].ExpiryDate.Month() == time.Now().Month() && itr.Coins[j].ExpiryDate.Year() == time.Now().Year() {
				...
				_, errSubtractCoins := httpClient.ParseClient("PUT", "http://localhost:8083/subtractCoinsLambda", coinSubtractBody, &resp)
			...
			if eventResponse.Results[j].EventDate.Day() == time.Now().Day() && eventResponse.Results[j].EventDate.Month() == time.Now().Month() {
				...
				for i := 0; i < len(userResponse.Results); i++ {
					...
					body := strings.NewReader(`{
						...
						"ruleId":"` + eventResponse.Results[j].ObjectID + `"
					}`)
					_, errCoins := httpClient.ParseClient("PUT", "http://localhost:8083/expendatureAddCoin", body, &resp)
					if errCoins != nil {
						return errCoins
					}
				}
	...
	if int(time.Now().Local().Weekday()) == 6 {
		...
		for i := 0; i < len(projects.Results); i++ {
			if projects.Results[i].ProjectID != prevProjectId {
				body := strings.NewReader(`{
					"projectId":"` + projects.Results[i].ProjectID + `"
				}`)
				_, err := httpClient.ParseClient("PUT", "http://localhost:8083/expendatureAddCoin", body, &resp)
				if err != nil {
					return err
				}
